...
Плагин ранжирования для Elasticsearch где более высокий score получают документы, у которых искомая фраза находится ближе к началу предложения с учетом расположения искомых term.

BM25 не подходит - нам нужно ранжировать набор документов после match: operator:  AND
...
Скрипт запускает локальный docker контейнер с разрешенным debug. 
